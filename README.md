...
git clone https://github.com/Yourl0calbroker/BrokerOSNIT.git $HOME/BrokerOSNIT && \
chmod +x $HOME/BrokerOSNIT/BrokerOSNIT.py $HOME/BrokerOSNIT/LocalOSNIT.py && \
mkdir -p $HOME/bin && \
if ! grep -q 'export PATH="$HOME/bin:$PATH"' $HOME/.bashrc; then echo 'export PATH="$HOME/bin:$PATH"' >> $HOME/.bashrc; fi && \
source $HOME/.bashrc && \
ln -sf $HOME/BrokerOSNIT/BrokerOSNIT.py $HOME/bin/BrokerOSNIT.py && \
ln -sf $HOME/BrokerOSNIT/LocalOSNIT.py $HOME/bin/LocalOSNIT.py
